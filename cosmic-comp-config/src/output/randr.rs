...
fn config_to_randr_output(
    ...
    cosmic_randr_shell::Output {
        ...
        transform: Some(match output.transform {
            ...
            crate::output::comp::TransformDef::Flipped90 => cosmic_randr_shell::Transform::Flipped90,
            crate::output::comp::TransformDef::Flipped180 => cosmic_randr_shell::Transform::Flipped180,
            crate::output::comp::TransformDef::Flipped270 => cosmic_randr_shell::Transform::Flipped270,
        }),
        modes: vec![current],
...
pub fn get_matching_config(
    ...
    let mut best_match: Option<(&Vec<super::comp::OutputInfo>, &Vec<super::comp::OutputConfig>)> = None;
    ...
    for (saved_infos, saved_configs) in output_config.config.iter() {
        ...
        let all_match = saved_infos.iter().all(|saved_info| {
            current_outputs.iter().any(|current| info_matches_output(saved_info, current))
        });
        ...
        if all_match {
            ...
            if best_match
                .as_ref()
                .map_or(true, |(infos, _)| infos.len() < saved_infos.len())
            {
