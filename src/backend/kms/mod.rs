...
impl State {
    ...
    // MERGE: our `perf_snapshots` / `push_pointer_input` helpers used to open an
    // `impl KmsState` block here; upstream moved `select_primary_gpu` and
    // `update_default_feedback` into `impl State` (dmabuf_global now lives on KmsState),
    // so those helpers moved down into the later `impl KmsState` block.
...
impl KmsState {
    ...
    pub fn recover_from_gpu_reset(&mut self) {
        ...
        // MERGE: dropped `blur::invalidate_all_blur_caches()` — upstream's blur captures the
        // live framebuffer each frame, so there is nothing to invalidate after a GPU reset.
    }
