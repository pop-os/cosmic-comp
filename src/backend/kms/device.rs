...
use cosmic_comp_config::output::{
    comp::{AdaptiveSync, OutputConfig, OutputState},
...
fn populate_modes(
    ...
    if new_config {
        let embedded = conn_info.interface() == connector::Interface::EmbeddedDisplayPort;
        let scale = conn_info
            .size()
            .map(|size| calculate_scale(embedded, size, mode.size()))
            .unwrap_or(1.0);
        ...
        *output_config = OutputConfig {
            ...
            max_bpc,
            scale,
            // Remember the heuristic's choice so settings / FTUE can surface it
            // as the "Recommended" scale even after the user overrides `scale`.
            recommended_scale: Some(scale),
