...
/// re-arms for the remainder if nothing applied yet.
...
/// Whether the seat is focusing a fullscreen surface. A fullscreen window that
/// is not focused can be stacked behind others, so its presence on the output
/// says nothing about what the user is looking at.
fn fullscreen_focused(seat: &Seat<State>) -> bool {
    matches!(
        seat.get_keyboard().and_then(|kbd| kbd.current_focus()),
...
fn on_idle_timer(state: &mut State, seat: &Seat<State>) -> TimeoutAction {
    let config = state.common.config.cosmic_conf.cursor_hide;
    let fullscreen = fullscreen_focused(seat);
    let cursor_state = seat.user_data().get::<CursorState>().unwrap();
