...
pub fn render_window_to_buffer(
        ...
        CosmicMappedRenderElement<R>: RenderElement<R>,
    {
        ...
        if let Ok(dmabuf) = get_dmabuf(buffer) {
            ...
            let states = dt.damage_output(age, &elements)?.1;

            let fb = offscreen.expect("shm buffer should have an offscreen target");
            dt.render_output_with_states(renderer, fb, 0, &elements, Color32F::TRANSPARENT, states)
        }
    }
...
pub fn render_cursor_to_buffer(
        ...
        CosmicMappedRenderElement<R>: RenderElement<R>,
    {
        ...
        if let Ok(dmabuf) = get_dmabuf(buffer) {
            ...
            dt.render_output_with_states(renderer, fb, 0, &elements, [0.0, 0.0, 0.0, 0.0], states)
        }
    }
