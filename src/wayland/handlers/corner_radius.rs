...
impl CornerRadiusHandler for State {
    ...
    // MERGE: dropped our `force_redraw` helper (scheduled a render for the toplevel's visible
    // output on corner-radius set/unset). Upstream moved corners/padding into double-buffered
    // surface state (CacheableCorners/CacheablePadding), so they apply on commit and the commit
    // path already schedules the redraw. The helper also no longer compiles: CornerRadiusData is
    // now keyed by CornerRadiusSurface (Toplevel/Popup/Layer) instead of a `toplevel` field.
