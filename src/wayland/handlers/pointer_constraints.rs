...
impl PointerConstraintsHandler for State {
    ...
    fn remove_constraint(
        ...
        // Smithay calls this with the constraint mutex held and, on the pointer-leave path,
        // from inside pointer dispatch. `apply_cursor_hint` retakes that mutex and drives the
        // pointer itself, so the warp has to happen once dispatch has unwound.
        ...
        self.common.event_loop_handle.insert_idle(move |state| {
            ...
            {
                state.apply_cursor_hint(&surface, &pointer, hint_location);
                seat.set_pointer_constraint_hint(None);
            }
        });
    }
