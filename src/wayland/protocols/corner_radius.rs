...
/// The surface's corner radii in logical pixels, clamped so the arcs cannot
/// overlap on the shorter side.
///
/// Ported across the move to upstream's corner-radius protocol, where the radii
/// widened from `u8` to `u32`.
pub fn get_surface_corner_radius(
    ...
    with_states(surface, |states| {
        ...
        let half_min_dim = u32::try_from(geometry_size.w.min(geometry_size.h) / 2).unwrap_or(0);
