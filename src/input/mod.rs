                ...
                if let Some(seat) = shell.seats.for_device(&event.device()).cloned() {
                    ...
                    if ptr.is_grabbed() {
                        ...
                        let new_keyboard_target =
                            State::element_under(position, &output, &*shell);
                    ...
                    if !seat.get_pointer().unwrap().is_grabbed() {
                        ...
                        if let Some(target) = under {
                            if let Some(surface) = target.toplevel().map(Cow::into_owned) {
                                if seat.get_keyboard().unwrap().modifier_state().logo && !shortcuts_inhibited {
                ...
                match stage {
                    Stage::SessionLock(lock_surface) => {
                        return ControlFlow::Break(Ok(
                            lock_surface.cloned().map(KeyboardFocusTarget::LockSurface)
                        ))
                    }
                    Stage::LayerPopup {
                        layer,
                        ...
                        if layer.can_receive_keyboard_focus() {
                            ...
                            .is_some()
                            {
                                return ControlFlow::Break(Ok(Some(KeyboardFocusTarget::LayerSurface(
                                    layer,
                                ))));
                            }
                        }
                    ...
                    Stage::StickyPopups(layout) => {
                        if let Some(element) = layout.popup_element_under(global_pos.to_local(output)) {
                            return ControlFlow::Break(Ok(Some(element)));
                        }
                    }
                    Stage::Sticky(layout) => {
                        if let Some(element) = layout.toplevel_element_under(global_pos.to_local(output)) {
                            return ControlFlow::Break(Ok(Some(element)));
                        }
                    }
