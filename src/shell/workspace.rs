...
impl Workspace {
    ...
    pub fn unmap_element(&mut self, mapped: &CosmicMapped) -> Option<WorkspaceRestoreData> {
        ...
        if let Some(tiling_state) = self.tiling_layer.unmap(&mapped, None) {
    ...
    pub fn popup_element_under(
        ...
        let has_focused_fullscreen = previous_focus
        ...
        if let Some(fullscreen) = self.fullscreen.as_ref() {
            if has_focused_fullscreen && !fullscreen.is_animating() {
    ...
    pub fn popup_surface_under(
        ...
        self.fullscreen
            .as_ref()
            .filter(|_| matches!(current_focus, Some(KeyboardFocusTarget::Fullscreen(_))))
    ...
    pub fn set_tiling(
        ...
        if tiling {
                ...
                .into_iter()
            {
                ...
                self.tiling_layer.unmap(&window, None);
        ...
        WorkspaceRenderElement<R>: RenderElement<R>,
    {
        ...
        let fullscreen_is_focused = last_active_seat.as_ref().is_some_and(|seat| {
            seat.get_keyboard()
                .unwrap()
                .current_focus()
                .is_some_and(|target| matches!(target, KeyboardFocusTarget::Fullscreen(_)))
        ...
        {
            let focused =
                last_active_seat.and_then(|seat| self.focus_stack.get(seat).last().cloned());
        ...
        WorkspaceRenderElement<R>: RenderElement<R>,
    {
                ...
                .unwrap_or(true)
        {
            ...
            elements.extend(
                self.tiling_layer
                    .render_popups::<R>(renderer, last_active_seat, zone, overview, theme)?
                    .into_iter()
                    .map(WorkspaceRenderElement::from),
            );
        }
