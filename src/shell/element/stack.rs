        ...
        C: From<CosmicStackRenderElement<R>>,
    {
        ...
        elements.extend(self.0.with_program(|p| {
            ...
            border.into_iter().chain(
                windows[active]
                    .render_elements::<R, CosmicStackRenderElement<R>>(
                        renderer,
                        window_loc,
                        scale,
                        alpha,
...
impl<R: Renderer + ImportAll + ImportMem> From<SurfaceRenderElement>
    for CosmicStackRenderElement<R>
{
    fn from(value: SurfaceRenderElement) -> Self {
        Self::Window(value)
    }
}
    ...
    R::Error: FromGlesError,
{
    fn draw(
        ...
        match self {
            ...
            CosmicStackRenderElement::Window(elem) => RenderElement::<GlowRenderer>::draw(
                elem,
                R::glow_frame_mut(frame),
                src,
                dst,
    ...
    fn capture_framebuffer(
        ...
        match self {
            CosmicStackRenderElement::Header(elem) => elem.capture_framebuffer(frame, src, dst),
            CosmicStackRenderElement::Shadow(elem) | CosmicStackRenderElement::Border(elem) => {
