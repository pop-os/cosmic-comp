...
use smithay::{
    ...
    utils::{IsAlive, Serial, SerialCounter, SERIAL_COUNTER},
    wayland::{
        seat::WaylandFocus,
...
impl Shell {
    ...
    /// Note: `pointer_check` is used to determine whether to update the pointer location if cursor_follows_focus is enabled
    ...
    pub fn set_focus(
        ...
        update_focus_state(seat, target, state, serial, check_update_cursor);
...
fn update_focus_state(
    ...
    if let Some(keyboard) = seat.get_keyboard() {
        if check_update_cursor && state.common.config.cosmic_conf.cursor_follows_focus {
            if ActiveFocus::get(seat).as_ref() != target && target.is_some() {
                ...
                let geometry = state
                    ...
                    .read()
                    .unwrap()
                    .focused_geometry(target.unwrap());
                //to avoid the nested mutable borrow of state
                if geometry.is_some() {
                    let top_left = geometry.unwrap().loc.as_logical().to_f64();

                    // create a pointer target from the target element
                    let pointer_target = PointerFocusTarget::from(target.unwrap().clone());

                    seat.get_pointer().unwrap().motion(
                        state,
                        Some((pointer_target, top_left)),
                        &MotionEvent {
                            location: top_left,
