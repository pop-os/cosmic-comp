...
fn render_input_order_internal<R: 'static>(
    ...
    for (layer, popup, location) in
        layer_popups(output, Layer::Overlay, element_filter)
    {
    ...
    if !has_fullscreen {
        for (layer, popup, location) in
            layer_popups(output, Layer::Top, element_filter)
        {
            callback(Stage::LayerPopup {
                layer,
                popup: &popup,
    ...
    if !has_fullscreen {
        // bottom layer popups
        for (layer, popup, location) in
            layer_popups(output, Layer::Bottom, element_filter)
        {
            callback(Stage::LayerPopup {
                layer,
                popup: &popup,
    ...
    if !has_fullscreen {
        ...
        for (layer, popup, location) in
            layer_popups(output, Layer::Background, element_filter)
        {
            callback(Stage::LayerPopup {
                layer,
                popup: &popup,
    ...
    if element_filter != ElementFilter::LayerShellOnly {
        ...
        if let Some((_, _, offset)) = previous.as_ref() {
            callback(Stage::Workspace {
                workspace,
                offset: *offset,
            })?;
        ...
        // TODO, cut them to their respective outputs for rendering / cursor checks
    }

    if !has_fullscreen {
        // bottom layer
        for (layer, mut location) in
            layer_surfaces(output, Layer::Bottom, element_filter)
        {
    ...
    if !has_fullscreen {
        // background layer
        for (layer, mut location) in
            layer_surfaces(output, Layer::Background, element_filter)
        {
            location += current_offset.as_global();
            callback(Stage::LayerSurface { layer, location })?;
        }
...
fn layer_surfaces<'a>(
    ...
    layers
        ...
        .map(|(surface, geometry)| {
            (surface, geometry.loc.as_local().to_global(output))
        })
}
