...
use smithay::{
    ...
    reexports::{
        ...
        =======
        */
        wayland_server::{DisplayHandle, Resource, Weak, protocol::wl_surface::WlSurface},
    },
    utils::{Buffer, IsAlive, Logical, Monotonic, Point, Rectangle, Serial, Time, Transform},
...
struct CursorImageStatusData {
    ...
    post_commit_hook: Option<(Weak<WlSurface>, HookId)>,
}
    ...
    fn drop(&mut self) {
        if let Some((surface, hook_id)) = &self.post_commit_hook {
            if let Ok(surface) = surface.upgrade() {
                remove_post_commit_hook(&surface, &hook_id);
            }
        }
    }
}
