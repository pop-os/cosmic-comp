...
use crate::{
    shell::layout::tiling::PlaceholderType,
    wayland::{
...
impl Shell {
    ...
    pub fn map_window(
        ...
        if let Some((surface, state, geometry)) = workspace.remove_fullscreen() {
        ...
        if should_be_fullscreen {
            workspace.map_fullscreen(&window, None, None);
            if was_activated {
    ...
    pub fn move_current_element<'a>(
        ...
        let workspace = self.active_space(&output).unwrap();
        let focus_stack = workspace.focus_stack.get(seat);
        let last = focus_stack.last().cloned();
        ...
        if let Some(surface) = fullscreen {
        ...
        } else if let Some(set) = last.as_ref().and_then(|last| {
            self.workspaces
                .sets
                .values_mut()
            ...
            if last
                ...
                .unwrap()
                .as_ref()
                .is_some_and(|state| matches!(state.original_layer, ManagedLayer::Tiling))
            {
    ...
    pub fn unmaximize_request(&mut self, mapped: &CosmicMapped) -> Option<Size<i32, Logical>> {
        if let Some(set) = self.workspaces.sets.values_mut().find(|set| {
            ...
            if let Some(state) = state.take() {
                assert!(matches!(state.original_layer, ManagedLayer::Sticky));
    ...
    where
        CosmicSurface: PartialEq<S>,
    {
        if let Some(mapped) = self.element_for_surface(surface).cloned() {
                ...
                .find(|(_, set)| set.sticky_layer.mapped().any(|m| m == &mapped))
            {
                ...
                toplevel_leave_output(&surface, &old_output);
                ...
                toplevel_enter_workspace(&surface, &workspace_handle);
